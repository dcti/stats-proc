...
#
# $Id: hourly.pl,v 1.106.2.12 2003/04/04 21:03:43 decibel Exp $
#
...
RUNPROJECTS: for (my $i = 0; $i < @statsconf::projects; $i++) {
  ...
  if( $qualcount > 0 ) {
	...
	if ($? == 0) {
      ...
      my $rowsnext = 0;
      my $sqlrows = 0;
      if(!open SQL, "psql -d $statsconf::database -f integrate.sql -v ProjectType=$project $statsconf::database -v HourNumber=$hh 2> /dev/stdout |") {

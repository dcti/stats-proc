#!/usr/bin/perl -w -I.
#
# $Id: load-day.pl,v 1.1 2003/09/11 02:04:01 decibel Exp $
#
...
stats::log($project,133,"$project log load has started");

my $maxdate = `sqsh -h -i dy_maxdate.sql`;
...
#    $dayrows = 0;
    ...
    for ($i = 0; $i < $innum; $i++) {
      ...
      $retcode = system "bcp", "import", "in", $unzipfn, "-ebcp_errors", "-SBLOWER", "-Ustatproc", "-PTR2cod#", "-c", "-t,";
      if ( $retcode > 0 ) {
        stats::log($project,139,"BCP Failed!");
        die;
      } else {
        my $recs = `sqsh -h -i dy_importcount.sql`;
        $recsval = 0+$recs;
...
#	die;
#    } else {
    stats::log($project,1,"Appened day's activity to master table. Log load finished");
#    }
#

#!/usr/bin/perl -Tw
#
# $Id: daily.pl,v 1.1 2003/09/11 02:04:01 decibel Exp $
#
...
if ("$insort[0]" =~ m/^rc5(\d\d\d\d\d\d\d\d)/ ) {
  ...
  if ( $earlyday >= $today ) {
    ...
    for ($i = 0; $i < $innum; $i++) {
      ...
      # bcp import in $1 -ebcp_errors -STALLY -Ustatproc -PSlfk39do -c -t, 
      $retcode = system "bcp", "import", "in", $unzipfn, "-ebcp_errors", "-STALLY", "-Ustatproc", "-PSlfk39do", "-c", "-t,";
      if ( $retcode > 0 ) {
